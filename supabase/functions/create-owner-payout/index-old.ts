...
serve(async (req) => {
  ...
  try {
    ...
    // Mode test SEULEMENT si explicitement défini ou si les credentials sont manquants en local
    const forceTestMode = Deno.env.get('FORCE_TEST_MODE') === 'true';
    const isLocalTest = forceTestMode || (!cinetpayTransferLogin || !cinetpayTransferPwd);
    
    // SÉCURITÉ : En production, refuser si les credentials CinetPay sont manquants
    if (!isLocalTest && (!cinetpayTransferLogin || !cinetpayTransferPwd)) {
      console.error(`[${timestamp}] [create-owner-payout] ERREUR: Variables CinetPay manquantes en production!`);
      throw new Error('Configuration CinetPay incomplète - impossible de traiter les payouts réels');
    }
...
async function doTransfer(
  ...
  try {
    ...
    if (contextData) {
      ...
      if (!ownerDataResult && bookingData?.fields?.owner_id) {
        ...
        if (ownerByBooking) {
          ...
          // 🔧 CORRECTION : Mettre à jour le payout avec le bon owner_id
          console.log(`[${timestamp}] [doTransfer] Correction owner_id du payout: ${payout.owner_id} → ${ownerByBooking.id}`);
          await supabase
            .from('payouts')
            .update({ owner_id: ownerByBooking.id })
            .eq('id', payout.id);
        }
      }
    ...
    if (isLocalTest) {
      console.warn(`[${timestamp}] [doTransfer] ⚠️  MODE TEST ACTIF - Simulation CinetPay (AUCUN TRANSFERT RÉEL!)`);
    ...
    if (isLocalTest) {
      console.warn(`[${timestamp}] [doTransfer] ⚠️  MODE TEST ACTIF - Simulation transfert (AUCUN ARGENT RÉEL ENVOYÉ!)`);
