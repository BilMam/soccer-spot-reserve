...
BEGIN
    ...
    -- IMPORTANT: Supprimer dans l'ordre des dépendances de clés étrangères
    
    -- Supprimer les payouts en premier (référence les bookings)
    ...
    -- Supprimer les notifications de réservation
    ...
    -- Supprimer les catégories d'avis
    ...
    -- Supprimer les comptes de paiement
    DELETE FROM public.payment_accounts 
    WHERE NOT (owner_id = ANY(user_ids_to_keep));
    
    -- Supprimer les comptes de payout
    DELETE FROM public.payout_accounts 
    WHERE NOT (owner_id = ANY(user_ids_to_keep));
    
    -- Supprimer les stats propriétaire
    DELETE FROM public.owner_stats 
    WHERE NOT (owner_id = ANY(user_ids_to_keep));
