-- Migration: Finalisation de l'automatisation des remboursements PayDunya
-- 1. Ajouter la colonne refund_metadata optionnelle
ALTER TABLE public.cagnotte_contribution
ADD COLUMN IF NOT EXISTS refund_metadata JSONB;

COMMENT ON COLUMN public.cagnotte_contribution.refund_metadata IS 'Métadonnées supplémentaires sur le remboursement (réponses API, etc.)';

-- 2. Finaliser cleanup_expired_cagnottes avec initialisation complète des compteurs
...
BEGIN
  ...
  FOR v_cagnotte_rec IN (
    ...
    ELSE
      ...
      -- Initialiser les remboursements avec compteurs à zéro
...
-- 3. Finaliser cancel_conflicting_cagnottes_in_progress avec initialisation complète des compteurs
